...
@section('body')
    ...
    <section class="pt-20 px-24">
            ...
            <div class="gap-2">
                <p class="text-xl mb-2">Paling sering dicari:
                    @foreach ($courses as $course)
                        <a href="#">
                            <span
                                class="bg-[#FFE7D2] text-[#FF8D3F] p-2 rounded text-[10px] font-bold hover:bg-[#FFF5EC]">{{ $course->category->name }}</span>
                        </a>
                    @endforeach
            </div>
            </p>
        ...
        <div class="py-8">
            ...
            @else
                <p class="py-8 text-xl font-semibold">Ditemukan ({{ count($courses) }})</p>
